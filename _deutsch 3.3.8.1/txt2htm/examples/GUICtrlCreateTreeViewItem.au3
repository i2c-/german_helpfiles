	...
	While 1
		...
		Select
			...
			Case $msg = $infobutton
				...
				If $item = 0 Then
					MsgBox(64, "TreeviewDemo", "Momentan ist kein Eintrag markiert")
				Else
					$text = GUICtrlRead($item, 1) ; Holt den Text des Treeview-Items
					If $text == "" Then
						MsgBox(16, "Fehler", "Fehler w&auml;hrend der Abfrage des Treeview-Items")
					Else
						MsgBox(64, "TreeviewDemo", "Aktuell gew&auml;hltes Item ist: " & $text) ; $advmsg[0] enth&auml;lt den Text und $advmsg[1] den Statuswert des Treeview-Items
					EndIf
				EndIf
