...
GUICreate("Datei Verschl&uuml;sseln", 425, 100)
...
Local $iEncrypt = GUICtrlCreateButton("Verschl&uuml;sseln", 345, 70, 75, 25)
GUISetState(@SW_SHOW)
	...
	Switch GUIGetMsg()
		...
		Case $iEncrypt
			...
			If StringStripWS($sSourceRead, 8) <> "" And StringStripWS($sDestinationRead, 8) <> "" And StringStripWS($sPasswordRead, 8) <> "" And FileExists($sSourceRead) Then ; Pr&uuml;fen ob eine Datei und ein Passwort gegeben sind.
				Local $iSuccess = _Crypt_EncryptFile($sSourceRead, $sDestinationRead, $sPasswordRead, $bAlgorithm) ; Verschl&uuml;sseln der Datei.
				If $iSuccess Then
