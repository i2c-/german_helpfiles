...
; Tabelle 'Test' ist jetzt besch&auml;ftigt...

_SQLite_SetTimeout($hDB_b, 0)
$iTimer = TimerInit()
$iRval = _SQLite_Exec($hDB_b, "SELECT * FROM Test") ; Das wird fehlschlagen!
MsgBox(0, "_SQLite_SetTimeout: Beispiel ohne Timeout", "Zeit: " & TimerDiff($iTimer) & @CRLF _
		 & "Fehler: " & _SQLite_ErrMsg($hDB_b) & @CRLF)
_SQLite_SetTimeout($hDB_b, 5000)
$iTimer = TimerInit()
$iRval = _SQLite_Exec($hDB_b, "SELECT * FROM test") ; Das wird fehlschlagen!
MsgBox(0, "_SQLite_SetTimeout: Beispiel mit 5s Timeout", "Zeit: " & TimerDiff($iTimer) & @CRLF _
		 & "Fehler: " & _SQLite_ErrMsg($hDB_b) & @CRLF)
