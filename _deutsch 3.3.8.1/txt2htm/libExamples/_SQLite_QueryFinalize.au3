...
_SQLite_FetchNames($hQuery, $aNames) ; Auslesen der Spalten&uuml;berschriften
MsgBox(0, "SQLite", "Spaltentitel: " & StringFormat(" %-10s  %-10s  %-10s  %-10s ", $aNames[0], $aNames[1], $aNames[2], $aNames[3]) & @CRLF)
While _SQLite_FetchData($hQuery, $aRow) = $SQLITE_OK ; Holt eine Zeile
	MsgBox(0, "SQLite", "Holt Daten mit FetchData: " & StringFormat(" %-10s  %-10s  %-10s  %-10s ", $aRow[0], $aRow[1], $aRow[2], $aRow[3]) & @CRLF)
