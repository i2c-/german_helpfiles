...
Func _WinGetCaretPos()
	...
	Local $aGetPos = WinGetPos("[ACTIVE]") ; Ermittelt sowohl die Position als auch die H&ouml;he und Breite des aktiven Fensters.
	Local $sControl = ControlGetFocus("[ACTIVE]") ; Ermittel den Controlnamen welches den Tastaturfokus besitzt.
	Local $aControlPos = ControlGetPos("[ACTIVE]", "", $sControl) ; Ermittelt die Position als auch die Gr&ouml;&szlig;e des Controls.
	$iOpt = Opt("CaretCoordMode", $iOpt) ; Setzt den "CaretCoordMode" zur&uuml;ck auf die vorherige Option.
	...
	If IsArray($aGetCaretPos) And IsArray($aGetPos) And IsArray($aControlPos) Then
		...
		Return $aReturn ; Gibt das Array zur&uuml;ck.
	Else
		Return SetError(1, 0, $aReturn) ; Gibt das Array zur&uuml;ck. und setzt @error auf 1.
	EndIf
