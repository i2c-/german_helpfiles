...
Func Example()
	...
	GUICreate("TreeView: Gibt das Item an den festgelegten Koordinaten zur&uuml;ck", 600, 300)
	...
	While 1
		Switch GUIGetMsg()
			...
			Case $GUI_EVENT_PRIMARYDOWN
				$a = GUIGetCursorInfo()
				_GUICtrlTreeView_SelectItem($hTreeView, _GUICtrlTreeView_HitTestItem($hTreeView, $a[0], $a[1]))
				MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("HitTestItem (%d, %d): %s", $a[0], $a[1], _GetHitString(_GUICtrlTreeView_HitTestItem($hTreeView, $a[0], $a[1]))))
		EndSwitch
	WEnd
	GUIDelete()
EndFunc   ;==>Example
