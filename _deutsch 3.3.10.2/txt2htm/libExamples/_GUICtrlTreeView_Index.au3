...
Func Example()
	...
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Index des Child-Items an Position %d (bei 0 beginnend):" & @CRLF & "%d", $iRand, _GUICtrlTreeView_Index($hTreeView, $hChildItem[$iRand])))

	$iRand = Random(0, 4, 1)
	_GUICtrlTreeView_SelectItem($hTreeView, $hItem[$iRand])
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Position (bei 0 beginnend) des Items mit dem Index %d:" & @CRLF & "%d", $iRand, _GUICtrlTreeView_Index($hTreeView, $hItem[$iRand])))
