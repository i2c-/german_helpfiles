...
Func Example()
	...
	For $x = 0 To 3
		...
		For $y = 0 To 4
			...
			For $z = 1 To 5
				$hChildItem[$iYItem + $z] = _GUICtrlTreeView_AddChild($hTreeView, $hChildItem[$iYItem], StringFormat("[%02d] Neues Child", $iYItem + $z), 0, 3)
			Next
			$iYItem += 6
		Next
	Next
	...
	$iRand = Random(0, 99, 1)
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Index %d Ebene: %d", $iRand, _GUICtrlTreeView_Level($hTreeView, $hChildItem[$iRand])))
