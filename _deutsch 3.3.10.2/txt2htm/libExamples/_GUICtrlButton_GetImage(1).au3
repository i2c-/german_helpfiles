...
Func Example()
	...
	$btn = GUICtrlCreateButton("Button 1", 10, 10, 40, 40, $BS_BITMAP)
	_GUICtrlButton_SetImage($btn, $sPath & "\blue.bmp")

	$chk = GUICtrlCreateCheckbox("Check 1", 60, 10, 50, 32, $BS_ICON)
	_GUICtrlButton_SetImage($chk, "shell32.dll", 14, True)

	$rdo = GUICtrlCreateRadio("Radio 1", 120, 10, 50, 32, $BS_ICON)
	_GUICtrlButton_SetImage($rdo, "shell32.dll", 21, True)

	MemoWrite("Button 1 Bild-Handle: " & _GUICtrlButton_GetImage($btn))
	MemoWrite("Check 1 Bild-Handle : " & _GUICtrlButton_GetImage($chk))
	MemoWrite("Radio 1 Bild-Handle : " & _GUICtrlButton_GetImage($rdo))
