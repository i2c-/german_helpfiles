...
Func Example()
	...
	MsgBox($MB_SYSTEMMODAL, "Information", "Tree f&uuml;r den Child-Index 2: " & _GUICtrlTreeView_GetTree($hTreeView, $hChildItem[2]))
	_GUICtrlTreeView_SelectItem($hTreeView, $hChildItem[2])

	$iRand = Random(0, 29, 1)
	_GUICtrlTreeView_SelectItem($hTreeView, $hChildItem[$iRand])
	MsgBox($MB_SYSTEMMODAL, "Information", StringFormat("Tree f&uuml;r den markierten Child-Index %d: %s", $iRand, _GUICtrlTreeView_GetTree($hTreeView)))
